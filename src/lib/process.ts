...
export function makeNoise(seedInt: number) {
...
export function glyphPath(slug: string): string {
...
export function settledFrameSvg(seed: string, amplitude: number[]): string {
  const W = 575, H = 136;
...
export function commitAmplitude(days: { date: string; count: number }[], todayIso: string) {
